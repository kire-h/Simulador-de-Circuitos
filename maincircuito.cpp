...
void MainCircuito::slotModificarPorta(int IdPort, QString TipoPort, int NumInputsPort)
{
  ...
  {
    // Altera a porta.
    // Gera excecao em caso de parametro invalido.
...
void MainCircuito::slotModificarConexao(int IdPort, int IdInput1, int IdInput2, int IdInput3, int IdInput4)
{
  ...
  try
  {
    // Altera as conexoes da porta.
    // Gera excecao se parametros invalidos.
    C.setIdInPort(IdPort, 0, IdInput1);
    if (C.getNumInputsPort(IdPort)>=2) C.setIdInPort(IdPort, 1, IdInput2);
    if (C.getNumInputsPort(IdPort)>=3) C.setIdInPort(IdPort, 2, IdInput3);
    if (C.getNumInputsPort(IdPort)>=4) C.setIdInPort(IdPort, 3, IdInput4);
    // Depois de alteradas, devem ser reexibidas as conexoes
    showConexao(IdPort);
  }
  catch(const std::exception &ex)
  {
...
void MainCircuito::slotModificarSaida(int IdSaida, int IdOrigemSaida)
{
  ...
  try
  {
    // Modifica a saida.
    // Gera excecao se parametros invalidos.
    C.setIdOutputCirc(IdSaida, IdOrigemSaida);
...
void MainCircuito::showPort(int IdPort)
{
  ...
  if (IdPort<1 || IdPort>C.getNumPorts())
  {
...
void MainCircuito::showOutput(int IdSaida)
{
  ...
  if (IdSaida<1 || IdSaida>C.getNumOutputs())
  {
